    ...
    ui(new Ui::management)
{
    ...
    //connect(ui->statsbutton, &QPushButton::clicked, this, &::management::on_statsbutton_clicked); // essallah
}
...
void management::on_export_table_clicked()
{
    ...
    // Write headers (exclude the last column)
    ...
    for (int col = 0; col < columnCount; ++col) {
        ...
        out << "\"" << headerText << "\"";  // Enclose in quotes for safety
        if (col < columnCount - 1)
            out << ",";  // Use ";" or "," based on your locale
    }
    out << "\n";

    // Write each row's data (exclude the last column)
    int rowCount = ui->tableWidget->rowCount();
    for (int row = 0; row < rowCount; ++row) {
        for (int col = 0; col < columnCount; ++col) {
            QTableWidgetItem *item = ui->tableWidget->item(row, col);
            out << "\"" << (item ? item->text() : "") << "\"";  // Enclose cell data in quotes
            if (col < columnCount - 1)
                out << ",";  // Use ";" or "," based on your locale
        }
