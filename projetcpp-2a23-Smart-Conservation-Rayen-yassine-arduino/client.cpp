...
bool client::update(int selectedClientId,QString name, int phone, QString email, QString adresse, int tot_ord)
{
    QSqlQuery query;
    QString idstr = QString::number(selectedClientId);
...
bool client::update(int id, QString name, int phone, QString email, QString adresse, int tot_ord)
{
    ...
    qDebug() << "ID1111111:" << id;
    // Convert integers to QString for binding
    QString idstr = QString::number(id);
    QString phonestr = QString::number(phone);
    QString tot_ordstr = QString::number(tot_ord);
    ...
    // Debugging outputs
    qDebug() << "ID:" << idstr;
    qDebug() << "Phone:" << phonestr;
    qDebug() << "Total Orders:" << tot_ordstr;
    ...
    if (email.indexOf('@') < 0 || email.indexOf('.') < email.indexOf('@') || email.indexOf('.') == email.length() - 1) {
        QMessageBox::warning(nullptr, "ERROR ADD", "Email must contain '@' and '.' after '@'.");
        return false;
    }

    // Prepare the SQL query
    query.prepare("UPDATE CLIENT SET NAME = :name, PHONE_NUMBER = :phone, EMAIL = :email, ADRESS = :adresse, TOTAL_ORDERS = :tot_ord WHERE CLIENT_ID = :id");
    query.bindValue(":id", idstr); // Correct placeholder
    query.bindValue(":name", name);
    query.bindValue(":phone", phonestr);
    query.bindValue(":email", email);
    query.bindValue(":adresse", adresse);
    ...
    if (!query.exec()) {
        ...
        QMessageBox::warning(nullptr, "Update Failed", "Failed to update client information.");
        return false;
    }
...
/*
void client::readclient(QTableView *tableView) {
    // Set up a new model for the table view
    QSqlQueryModel *model = new QSqlQueryModel();
    QSqlQuery query;

    // Prepare the query to fetch all clients
    query.prepare("SELECT * FROM CLIENT");
    ...
    if (query.exec()) {
        ...
        // Adjust table view properties (optional)
        tableView->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);
        tableView->verticalHeader()->setSectionResizeMode(QHeaderView::ResizeToContents);
        tableView->setSortingEnabled(true);  // Enable sorting

    } else {
        qDebug() << "Failed to execute readclient query:" << query.lastError().text();
    }
}*/
...
void client::readclient(QTableView *tableView) {
    ...
    if (query.exec()) {
        ...
        while (query.next()) {
            ...
            // Convert CLIENT_ID and PHONE_NUMBER to strings to prevent scientific notation
            QString idClient = QString::number(query.value(0).toLongLong());  // CLIENT_ID as string
            QString phoneNumber = query.value(2).toString();  // PHONE_NUMBER as string

            // Handle large numbers and ensure they display fully without scientific notation
            // For CLIENT_ID, we use QString::number() to convert it to a string.
            // For PHONE_NUMBER, we do the same to ensure it's displayed properly.

            rowItems << new QStandardItem(idClient);  // Id Client
            rowItems << new QStandardItem(query.value(1).toString());  // Name
            rowItems << new QStandardItem(phoneNumber);  // Phone Number
            rowItems << new QStandardItem(query.value(3).toString());  // E-mail
            rowItems << new QStandardItem(query.value(4).toString());  // Address
            rowItems << new QStandardItem(query.value(5).toString());  // Total Orders
...
QStandardItemModel* client::rechercher(const QString &type, const QString &searchText) {
    QStandardItemModel *model = new QStandardItemModel();
    QSqlQuery query;

    qDebug() << "Searching for:" << searchText << "with type:" << type;  // Debugging search params

    // Prepare query based on search type
    if (type == "Name") {
        query.prepare("SELECT * FROM CLIENT WHERE NAME LIKE :searchText");
        query.bindValue(":searchText", "%" + searchText + "%");
    } else if (type == "Client_id") {
        query.prepare("SELECT * FROM CLIENT WHERE CLIENT_ID LIKE :searchText");
        query.bindValue(":searchText", "%" + searchText + "%");
    } else {
        qDebug() << "Invalid search type!";
        return model;  // Return empty model if type is invalid
    }

    // Execute the query after binding the values
    if (!query.exec()) {
        qDebug() << "Query Error:" << query.lastError().text();
        return nullptr; // Return nullptr if query failed
    }
    ...
    while (query.next()) {
        ...
        // Convert CLIENT_ID to string to avoid scientific notation
        QString idClient = query.value(0).toString();  // CLIENT_ID as string
        rowItems << new QStandardItem(idClient);  // Id Client

        // Convert other columns to string and add to row
        rowItems << new QStandardItem(query.value(1).toString()); // Name
        rowItems << new QStandardItem(query.value(2).toString()); // Phone Number
        rowItems << new QStandardItem(query.value(3).toString()); // Email
        rowItems << new QStandardItem(query.value(4).toString()); // Address
        rowItems << new QStandardItem(query.value(5).toString()); // Total Orders
    ...
    // Debugging the number of rows returned
    qDebug() << "Rows found:" << model->rowCount();
