...
#include <QInputDialog>  // Include for input dialog
#include <QSerialPort>    // Include for serial communication
...

{
    ...
    //-----------arduinoFM
    initializePeriodicUpdates();  // Start periodic updates for progress bars

    ui->factory_2->setCurrentWidget(ui->factory_2);
    ui->TEMP->setRange(0, 100); // Example range for temperature
    ui->HUM->setRange(0, 100); // Example range for humidity
    // Connect to Arduino
    if (arduinoFM.connect_arduinoFM() == 0) {
    ...
    connect(ui->stackedWidget, &QStackedWidget::currentChanged, this, [this](int index)
            {
                if (index == 5) { // Maintenance planning page
                    updateCalendarHighlights();
                    loadMaintenanceDates(); // Load maintenance dates when the page is selected
                }
            });


    connect(ui->tableView_5, &QTableView::clicked, this, &management::showMaintenanceCalendar);



    // Ajout des items à la liste de diagnostics
    QStringList diagnostics = {
        "Surchauffe", "Perte de puissance", "Bruit inhabituel",
        "Arrêts fréquents", "Décharge rapide de la batterie",
        "Écran non fonctionnel", "Étincelles électriques",
        "Opération lente", "Vibrations"
    };
    ui->listWidget_4->addItems(diagnostics);
    // Connexions des boutons de navigation
    connect(ui->buttonAddNewMachine, &QPushButton::clicked, this, &::management::goToAddNewMachine);
    connect(ui->buttonUpdateMachine, &QPushButton::clicked, this, &::management::goToUpdateMachine);
    connect(ui->buttonDiagnosis, &QPushButton::clicked, this, &::management::goToDiagnosis);
    connect(ui->buttonPlanMaintenance, &QPushButton::clicked, this, &::management::goToPlanMaintenance);
    connect(ui->buttonBackFromAdd, &QPushButton::clicked, this, &::management::goToHomePage);
    connect(ui->buttonBackFromUpdate, &QPushButton::clicked, this, &::management::goToHomePage);
    connect(ui->buttonBackFromDiagnosis, &QPushButton::clicked, this, &::management::goToHomePage);
    connect(ui->buttonBackFromMaintenance, &QPushButton::clicked, this, &::management::goToHomePage);

    // Connect Buttons
    connect(ui->pushButton_13, &QPushButton::clicked, this, &management::on_pushButton_13_clicked);
    connect(ui->pushButton_26, &QPushButton::clicked, this, &management::on_pushButton_26_clicked);
    ...
    //--------------------------------factory------------------//
    ui->client->setCurrentWidget(ui->factory_2);

    ui->tableView_F_2->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);
    ui->tableView_F_2->verticalHeader()->setSectionResizeMode(QHeaderView::ResizeToContents);
    ui->tableView_F_2->setSortingEnabled(true);

    ui->tableView_F_2->setModel(fa.afficher());
    // Connecting button clicks to their respective slots
    connect(ui->add_2, &QPushButton::clicked, this, &management::on_add_clickedF);
    connect(ui->annuler_6, &QPushButton::clicked, this, &management::on_annuler_clickedF);
    connect(ui->sup_3, &QPushButton::clicked, this, &management::on_sup_clickedF);
    connect(ui->ajout_f_2, &QPushButton::clicked, this, &management::on_ajout_cl_2_clickedF);
    connect(ui->PDF_2, &QPushButton::clicked, this, &management::on_pdfButton_clickedF); // Connect PDF button
    connect(ui->filterButton_2, &QPushButton::clicked, this, &management::on_filterButton_clickedF);
    connect(ui->searchLocation_2, &QLineEdit::textChanged, this, &management::on_searchLocation_textChangedF);
    connect(ui->generateQRCodeButton_2, &QPushButton::clicked, this, &management::on_generateQRButton_clickedF);
    connect(ui->displayQRButton_2, &QPushButton::clicked, this, &management::on_displayQRButton_clickedF);




    connect(ui->add_2, &QPushButton::clicked, this, &management::on_add_2_clicked);
    connect(ui->annuler_6, &QPushButton::clicked, this, &management::on_annuler_6_clicked);
    connect(ui->sup_3, &QPushButton::clicked, this, &management::on_sup_3_clicked);
    connect(ui->ajout_f_2, &QPushButton::clicked, this, &management::on_ajout_f_2_clicked);
    connect(ui->PDF_2, &QPushButton::clicked, this, &management::on_pdfButton_clickedF); // Connect PDF button
    connect(ui->filterButton_2, &QPushButton::clicked, this, &management::on_filterButton_2_clicked);
    connect(ui->searchLocation_2, &QLineEdit::textChanged, this, &management::on_searchLocation_2_textChanged);
    connect(ui->generateQRCodeButton_2, &QPushButton::clicked, this, &management::on_generateQRCodeButton_2_clicked);
    connect(ui->displayQRButton_2, &QPushButton::clicked, this, &management::on_displayQRButton_2_clicked);
    connect(ui->setthreshold,&QPushButton::clicked, this, &management::on_setThresholdButton_clicked);
    connect(ui->saveTMToButton,&QPushButton::clicked, this, &management::on_saveTMToButton_clicked);
...
void management::updateProgressBars() {
    ...
    ui->progressBar4_2->setValue(totalCount);

    ui->progressBar1_2->setValue(technicianPercentage);
    ui->progressBar2_2->setValue(chefCuisinePercentage);
    ui->progressBar3_2->setValue(managerPercentage);
...
void management::on_export_table_clicked()
{
    ...
    // headers ta3 table (excluding the last column)
    int columnCount = ui->tableWidget->columnCount() - 1; // Exclude last column
    for (int col = 0; col < columnCount; ++col) {
    ...
    // Write the table content row by row (excluding the last column)
...
void management::goToAddNewMachine()
{
    ui->stackedWidget->setCurrentIndex(1); // Page "Ajouter une machine"
}

void management::goToUpdateMachine()
{
    // Vérification si une ligne est sélectionnée
    QModelIndexList selectedRows = ui->tableView->selectionModel()->selectedRows();
    if (selectedRows.isEmpty()) {
        QMessageBox::warning(this, tr("Erreur"), tr("Veuillez sélectionner une machine avant de continuer."));
        return;
    }
    ui->stackedWidget->setCurrentIndex(2); // Page "Mettre à jour une machine"
}

void management::goToDiagnosis()
{
    ui->stackedWidget->setCurrentIndex(3); // Page "Diagnostic"
}

void management::goToPlanMaintenance()
{
    ui->stackedWidget->setCurrentIndex(4); // Page "Plan de maintenance"
}

void management::goToHomePage()
{
    ui->stackedWidget->setCurrentIndex(0); // Page "Accueil"
}
...
void management::on_ajouter_clicked()
{
    ...
    QString machineStatus = ui->comboBox_status_2->currentText();
...
void management::on_delete_2_clicked()
{
    QModelIndexList selectedRows = ui->tableView->selectionModel()->selectedRows();
    if (selectedRows.isEmpty()) {
        QMessageBox::warning(this, tr("Avertissement"), tr("Aucune machine sélectionnée."));
        return;
    }

    int machineID = ui->tableView->model()->data(selectedRows.first()).toInt();
    QMessageBox::StandardButton reply = QMessageBox::question(
        this, tr("Confirmation"), tr("Êtes-vous sûr de vouloir supprimer cette machine ?"),
        QMessageBox::Yes | QMessageBox::No
        );

    if (reply == QMessageBox::Yes && ma.supprimer(machineID)) {
        QMessageBox::information(this, tr("Succès"), tr("Machine supprimée avec succès."));
        ui->tableView->setModel(ma.afficher());
    } else {
        QMessageBox::critical(this, tr("Erreur"), tr("Échec de la suppression de la machine."));
    }
}
...
{
    int machineID = ui->lineEdit_9->text().toInt();
    bool success = ma.search(machineID);
    if (success) {
        QMessageBox::information(this, tr("Succès"), tr("Machine trouvée."));
    } else {
        QMessageBox::critical(this, tr("Erreur"), tr("Impossible de trouver la machine."));
    }
}
*/
// Fonction pour mettre à jour les informations d'une machine
void management::on_pushButton_26_clicked()
{
    // Vérifier si une ligne est sélectionnée dans le tableau
    QModelIndexList selectedRows = ui->tableView->selectionModel()->selectedRows();
    if (selectedRows.isEmpty()) {
        // Si aucune machine n'est sélectionnée, afficher un message d'erreur
        QMessageBox::warning(this, tr("Erreur"), tr("Veuillez sélectionner une machine dans le tableau avant de continuer."));
        return; // Arrêter l'exécution de la fonction
    }

    // Récupérer l'ID de la machine sélectionnée
    QModelIndex selectedIndex = selectedRows.first();
    int MachineID = ui->tableView->model()->data(ui->tableView->model()->index(selectedIndex.row(), 0)).toInt();

    // Charger les données de la machine sélectionnée
    QSqlQuery query;
    query.prepare("SELECT SERIAL_NUMBER, FACTORY_ID, TYPE, MACHINE_STATUS, LAST_MAINTENANCE_DATE FROM MACHINE WHERE MACHINEID = :MachineID");
    query.bindValue(":MachineID", MachineID);

    if (query.exec() && query.next()) {
        // Remplir les champs du formulaire avec les données existantes
        ui->lineEdit_SerialNumber_6->setText(query.value("SERIAL_NUMBER").toString());
        ui->lineEdit_Factory_6->setText(query.value("FACTORY_ID").toString());
        ui->comboBox_type_4->setCurrentText(query.value("TYPE").toString());
        ui->comboBox_status_4->setCurrentText(query.value("MACHINE_STATUS").toString());
        ui->lineEdit_Maintenance_6->setText(query.value("LAST_MAINTENANCE_DATE").toString());

        // Vérifier si la mise à jour des données est réussie
        bool success = ma.update(MachineID,
                                 ui->lineEdit_SerialNumber_6->text().toInt(),
                                 ui->lineEdit_Factory_6->text().toInt(),
                                 ui->comboBox_type_4->currentText(),
                                 ui->comboBox_status_4->currentText(),
                                 ui->lineEdit_Maintenance_6->text());

        if (success) {
            // Si la mise à jour est réussie
            QMessageBox::information(this, tr("Succès"), tr("La machine a été mise à jour avec succès."));
        } else {
            // Si la mise à jour échoue
            QMessageBox::critical(this, tr("Erreur"), tr("Impossible de mettre à jour la machine."));
        }

        // Naviguer vers la page de mise à jour
        goToUpdateMachine();
    } else {
        // Si la requête échoue ou que les données ne sont pas trouvées
        QMessageBox::critical(this, tr("Erreur"), tr("Impossible de charger les données de la machine."));
    }
}
...
void management::on_tableView_4_clicked(const QModelIndex &index)
{
    ...
    if (column == 3) { // "MACHINE_STATUS" is in the 4th column (index 3)
        ...
        int machineId = ui->tableView_5->model()->data(ui->tableView_5->model()->index(row, 0)).toInt();
        ...
        if (calendar->exec() == QDialog::Accepted) {
            ...
            machine m;
            ui->tableView_5->setModel(m.getScheduledMaintenance());
...
void management::showMaintenanceCalendar(const QModelIndex &index)
{
    ...
    if (index.column() == 3) { // 3 is the MACHINE_STATUS column
        // Get the machine ID from the first column of the same row
        int machineId = ui->tableView_5->model()->index(index.row(), 0).data().toInt();
...
void management::highlightMaintenanceDate(const QDate& date)
{
    ...
    ui->calendarWidget_4->setDateTextFormat(date, format);
}
...
void management::updateCalendarHighlights()
{
    ...
    ui->calendarWidget_4->setDateTextFormat(QDate(), QTextCharFormat());
    maintenanceDateToMachineIds.clear();

    QSqlQuery query;
    query.prepare("SELECT MACHINEID, NEXT_MAITENANCE_DATE FROM MACHINE WHERE NEXT_MAITENANCE_DATE IS NOT NULL");
...
void management::diagnose(int selectedMachineId)
{
    ...
    for (int i = 0; i < ui->listWidget_4->count(); ++i)
    {
        if (ui->listWidget_4->item(i)->checkState() == Qt::Checked) {
            selectedSymptoms.append(ui->listWidget_4->item(i)->text());
        }
    }
    ...
    ui->textEdit_3->setPlainText(diagnosticMessage);
}

void management::on_pushButton_31_clicked()
{
    QString machineId = ui->lineEdit_15->text();
    QList<QListWidgetItem*> selectedItems = ui->listWidget_4->selectedItems();

    if (machineId.isEmpty()) {
        ui->textEdit_3->setText("Please enter or select a valid Machine ID.");
        return;
    }

    if (selectedItems.isEmpty()) {
        ui->textEdit_3->setText("No symptoms selected for diagnosis. Please select one or more symptoms.");
        return;
    }
    ...
    ui->textEdit_3->setText(diagnosis);
}
...
{
    ui->main->setCurrentWidget(ui->add_factory_2);
}

void management::on_annuler_clickedF()
{
    ui->main->setCurrentWidget(ui->principale_5);
}

void management::on_sup_clickedF()
{
    ui->main->setCurrentWidget(ui->delate);
}
...
{
    ui->main->setCurrentWidget(ui->update_8);
}
...
void management::on_ajout_cl_2_clickedF()
{
    ...
    if (fa.search(idf) && fa.getFactoryData(idf)) {
        ...
        ui->chi_3->setText(QString::number(fa.getidc()));
        ui->totm_3->setText(QString::number(fa.gettot_m()));
        ui->tote_3->setText(QString::number(fa.gettot_e()));
        ui->cap_3->setText(QString::number(fa.getcapacity()));
        ui->loc_3->setText(fa.getlocation());

        // Switch to update page
        ui->factory_2->setCurrentWidget(ui->rupdate);
    } else {
...
{
    int idf = ui->search_6->text().toInt();
    int idc = ui->chi_3->text().toInt();
    int tot_m = ui->totm_3->text().toInt();
    int tot_e = ui->tote_3->text().toInt();
    int capacity = ui->cap_3->text().toInt();
    QString location = ui->loc_3->text();
...
{
    int id = ui->delate_f_2->text().toInt();
...
{
    int idf = ui->idf_3->text().toInt();
    int idc = ui->idc_3->text().toInt();
    int tot_m = ui->totm_3->text().toInt();
    int tot_e = ui->tote_3->text().toInt();
    int capacity = ui->cap_3->text().toInt();
    QString location = ui->loc_3->text();
...
{
    QString selectedCriteria = ui->filterComboBox_2->currentText(); // Critère sélectionné
    ...
    if (!query.isEmpty()) {
        ...
        ui->tableView_F_2->setModel(model); // Mettez à jour l'affichage
    }
}
...
void management::on_searchLocation_textChangedF(const QString &searchText)
{
    ...
    ui->tableView_F_2->setModel(model); // Update the table view to show filtered results
}
...
void management::updateFactoryCountF()
{
    ...
    if (query.next()) {
        ...
        // Update the label to show the total factory count
        ui->totalFactoriesLabel_2->setText(QString("Total Factories: %1").arg(count));

        // Set the progress bar range and update its value
        int maxFactories = 100; // Define the maximum number of factories for the progress bar
        ui->progressBar4_4->setRange(0, maxFactories); // Set the progress bar range
        ui->progressBar4_4->setValue(count);           // Set the progress bar to the current count
    }
}
...
void management::statistique() {
    ...
    ui->presenceBar1__10->setValue(totalCount);
    ui->progressBar2_5->setValue(totalorderPercentage);
...
void management::on_add_2_clicked()
{
    ui->factory_2->setCurrentWidget(ui->add_factory_2);
}


void management::on_ajout_f_2_clicked()
{
    int idf = ui->idf_3->text().toInt();
    int idc = ui->idc_3->text().toInt();
    int tot_m = ui->totmach_3->text().toInt();
    int tot_e = ui->totemp_4->text().toInt();
    int capacity = ui->capacity_3->text().toInt();
    QString location = ui->location_3->text();
...
void management::on_annuler_6_clicked()
{
    ui->factory_2->setCurrentWidget(ui->principale_5);
}


void management::on_sup_3_clicked()
{
    ui->factory_2->setCurrentWidget(ui->delate);
}


void management::on_delete_5_clicked()
{
    int id = ui->delate_f_2->text().toInt();
...
void management::on_cancel_6_clicked()
{
    ui->factory_2->setCurrentWidget(ui->principale_5);
}


void management::on_update_4_clicked()
{
    ui->factory_2->setCurrentWidget(ui->rupdate);
}


void management::on_update_7_clicked()
{
    int idf = ui->search_6->text().toInt();
    int idc = ui->chi_3->text().toInt();
    int tot_m = ui->totm_3->text().toInt();
...
void management::on_annuler_3_clicked()
{
    ui->factory_2->setCurrentWidget(ui->principale_5);
}
...
void management::on_staticbutton_clicked()
{
    // Get the selected location from the UI
    QString selectedLocation = ui->locationComboBox->currentText();  // Assuming there's a ComboBox for location
...
void management::on_filterButton_2_clicked()
{
    QString selectedCriteria = ui->filterComboBox_2->currentText(); // Critère sélectionné
...
void management::on_generateQRCodeButton_2_clicked()
{
    // Ask for the factory ID before proceeding
    bool ok;
    int factoryID = QInputDialog::getInt(this, tr("Enter Factory ID"),
...
void management::on_displayQRButton_2_clicked()
{
    // This function reuses the logic from on_generateQRButton_clicked.
    on_generateQRButton_clickedF();
}
...
void management::on_searchLocation_2_textChanged(const QString &searchText)
{
    QString query = "SELECT * FROM FACTORY WHERE LOCATION LIKE '%" + searchText + "%'";
...
void management::on_PDF_2_clicked()
{
    // Call the PDF export function from the factory class
    fa.pdf_downloader();
...
//----------------arduinoFM
void management::initializePeriodicUpdates()
{
    // Initialize the timer for periodic updates (e.g., every 1000 ms = 1 second)
    progressBarUpdateTimer = new QTimer(this);

    // Connect the timer's timeout signal to updateProgressBarsFM() method
    connect(progressBarUpdateTimer, &QTimer::timeout, this, &management::updateProgressBarsFM);

    // Start the timer to call updateProgressBarsFM() every 1 second (1000 ms)
    progressBarUpdateTimer->start(1000);
}
void management::updateProgressBarsFM()
{
    // Read data from Arduino
    QByteArray rawData = arduinoFM.read_from_arduinoFM();
    ...
    if (dataList.size() == 3) {
        bool tempOk, humOk;
        float temperature = dataList[1].toFloat(&tempOk);
        float humidity = dataList[2].toFloat(&humOk);

        if (tempOk && humOk) {
            // Update the progress bars with the new values
            ui->TEMP->setValue(static_cast<int>(temperature));
            ui->progressBar4_5->setValue(static_cast<int>(temperature)); // TEMP progress bar
            ui->HUM->setValue(static_cast<int>(humidity));
            ui->progressBar4_6->setValue(static_cast<int>(humidity));              // HUM progress bar

            // Optionally log the values
            qDebug() << "Temperature:" << temperature << "Humidity:" << humidity;
        } else {
            qDebug() << "Error parsing temperature or humidity.";
        }
    } else {
        qDebug() << "Received invalid data format:" << dataString;
    }
}
void management::on_setThresholdButton_clicked()
{
    // Ask the user to input a temperature threshold
    bool ok;
    double threshold = QInputDialog::getDouble(this, tr("Set Temperature Threshold"),
                                               tr("Enter Threshold Temperature (°C):"),
                                               24.0, 0.0, 100.0, 1, &ok);

    if (ok) {
        // Send the threshold value to the Arduino
        QByteArray thresholdData = QByteArray::number(threshold);

        // Open the serial port if not already open
        if (!arduinoFM.getserialFM()->isOpen()) {
            arduinoFM.getserialFM()->setPortName(arduinoFM.getarduino_port_nameFM());
            arduinoFM.getserialFM()->open(QIODevice::WriteOnly);
            arduinoFM.getserialFM()->setBaudRate(QSerialPort::Baud9600);
        }

        // Send the threshold to Arduino
        arduinoFM.getserialFM()->write(thresholdData);

        // Optionally display a message to the user
        QMessageBox::information(this, tr("Threshold Set"), tr("Temperature threshold set to %1°C").arg(threshold));
    }
}
void management::on_saveTMToButton_clicked()
{
    // Ask the user to enter the Factory ID
    bool ok;
    int factoryId = QInputDialog::getInt(this, tr("Enter Factory ID"),
                                         tr("Enter the Factory ID to save data:"), 0, 0, 99999, 1, &ok);

    if (ok) {
        // If the user entered a valid ID, proceed to save temperature and humidity data
        QByteArray rawData = arduinoFM.read_from_arduinoFM();
        QString dataString = QString::fromUtf8(rawData).trimmed();

        // Split data into multiple records based on newline (\r\n) separator
        QStringList records = dataString.split("\r\n");

        // Remove any empty records
        records.removeAll("");

        // Iterate over each record to process temperature and humidity
        for (const QString& record : records) {
            // Split each record by comma
            QStringList dataList = record.split(',');
            ...
            if (dataList.size() == 3) {
                ...
                if (tempOk && humOk) {
                    // Check if the factory ID exists in the database
                    QSqlQuery checkQuery;
                    checkQuery.prepare("SELECT COUNT(*) FROM FACTORY WHERE factory_id = ?");
                    checkQuery.addBindValue(factoryId);
                    ...
                    if (checkQuery.exec()) {
                        ...
                        if (count > 0) {
                            // Factory ID exists, update the existing record with new temperature and humidity
                            QSqlQuery updateQuery;
                            updateQuery.prepare("UPDATE FACTORY SET temperature = ?, humidity = ? WHERE factory_id = ?");
                            updateQuery.addBindValue(temperature);
                            updateQuery.addBindValue(humidity);
                            updateQuery.addBindValue(factoryId);

                            if (updateQuery.exec()) {
                                QMessageBox::information(this, tr("Data Updated"), tr("Temperature and Humidity data updated successfully."));

                                // Refresh the table view
                                ui->tableView_F_2->setModel(fa.afficher());  // Assuming fa.afficher() fetches the updated data
                                updateFactoryCountF();  // If you have a function that updates the factory count, call it
                            } else {
                                QMessageBox::warning(this, tr("Error"), tr("Failed to update data: %1").arg(updateQuery.lastError().text()));
                            }
                        } else {
                            // Factory ID does not exist, inform the user
                            QMessageBox::warning(this, tr("Factory ID Not Found"), tr("The factory ID does not exist in the database."));
                        }
                    } else {
                        QMessageBox::warning(this, tr("Error Checking Factory ID"), tr("Failed to check if factory exists: %1").arg(checkQuery.lastError().text()));
                    }
                } else {
                    QMessageBox::warning(this, tr("Invalid Data"), tr("Error parsing temperature or humidity."));
                }
            } else {
                QMessageBox::warning(this, tr("Invalid Data Format"), tr("Received invalid data from Arduino."));
            }
        }
    }
}
